...
- Discord's `/profile @user` (see `module/verification.yml`'s `discord.profile`) relays a profile lookup to the connected [MakongVelocity](../MakongVelocity) companion the same way `/makongcore ping` does, for the two things only the proxy can answer: nLogin registration/last-login and true whole-network online status. Without a connected Velocity companion, `/profile` still works - those three fields just show as unknown/offline.
- Every poll also reports this server's own live player count, and gets back every connected server's online state, player count, and how long it's been continuously online - what Discord's `/status` and the live status embed (see `module/verification.yml`'s `discord.status`) are built from.

This otherwise doesn't affect MaTier, Teams, or account linking - it's just a command/ping/profile-lookup/rankings-reporting channel to and from the website.
...
## 1.2.29 changes
- Added Discord's `/profile @user` - posts a generated profile card (Player name + online dot, MaTier rank/tier/Star, Team, account type, Time Registered, Last Login, and a live skin render) for whichever Minecraft account that Discord user has linked. Open to everyone, read-only. The card is rendered entirely in-process (`ProfileCard`, plain `java.awt`/`Graphics2D` - no new dependency) using the network's own bundled pixel font, so it needs no font or emoji support on the server itself. Online status and registration/last-login data are whole-network, relayed from the connected [MakongVelocity](../MakongVelocity) companion (nLogin + its own connected-player list) through the same website-bridge channel `/makongcore ping` uses - without Velocity connected, `/profile` still works, those three fields just show as unknown. New `discord.profile.tagline` in `module/verification.yml` sets the card's quote line. See `DOCUMENTATION.md`'s new §9.3.
...
## 1.2.32 changes
- Reorganized `module/verification.yml` for multi-server setups - no functional/behavior change, same keys and defaults, just reordered and re-commented. Added a "QUICK REFERENCE" checklist at the top of the file listing exactly which settings need to be set the same on every server (`discord.enabled`/`invite`, all of `telegram.*`, all of `linking.*`) versus which only ever matter on the one server with `discord.hub: true` (`discord.bot_token`, `discord.guild.*`, `discord.verification.*`, `discord.roles.*`, `discord.commands.*`, `discord.profile.*`) - only the hub server ever has a live Discord connection, so only it ever receives the interactions those settings are read from. Within `discord:` itself, `enabled`/`invite` (every-server) now come before `hub` and everything hub-only. Also flagged a real gap while at it: unlike `discord.hub`, Telegram has no hub concept at all yet - every server with `telegram.enabled: true` runs its own independent poller against the same bot token, so only enable it on one server in a multi-server network for now. As always, this only reorganizes the shipped default for a fresh install - an existing `verification.yml` keeps its current key order (the self-healing config only adds missing keys, never moves existing ones).
...
## 1.2.35 changes
**Breaking change - manual `module/team.yml` edit required on existing servers.** Team Stars now work like MaTier's player Stars: awarded/deducted straight to the team in real time on every kill, death and hour of playtime, instead of accumulating "Weekly Points" all week that get converted into Stars by a weekly payout. Unlike MaTier, teams still have no named tier ladder (no M1-M9 equivalent) - just a raw Star count and leaderboard. The annual Team Star reset (`team.annual_star_reset`) is unchanged and still runs on its own schedule.
- Removed the entire weekly points/rewards system: `team.weekly_points` (reset schedule) and `team.weekly_rewards` (the Sunday Star payout to the top 20 teams) are gone from `module/team.yml`, along with the `/mateam addweeklypoints`/`setweeklypoints` commands (and their `addpoints`/`setpoints` aliases). `WeeklyRewardService` is replaced by the much smaller `TeamStarResetService`, which only handles the annual Star reset it previously shared a class with.
- `team.scoring.minimum_points` is renamed to `team.scoring.minimum_stars` and every `team.scoring.events.*` value is now a Star amount awarded/deducted immediately, not a weekly point.
- `ConfigUpdater` only adds missing keys, it never removes or renames existing ones - existing servers keep their old (now-unused) `weekly_points`/`weekly_rewards`/`scoring.minimum_points` keys sitting inertly in `module/team.yml` until an admin deletes them manually. Regenerate the file from the shipped default, or copy the new `scoring` block and delete `weekly_points`/`weekly_rewards`, to fully clean it up.
- Team stats no longer track "points" at all - `Team`/`TeamMember` dropped the field entirely (kills, deaths and playtime are untouched). The database's `points` column is left in place (unused, always `0`) rather than dropped, consistent with this codebase's additive-only schema approach; no migration is needed.
- GUI: every "Weekly Points" label became "Stars" (or was removed where Stars were already shown alongside it on the same screen). The team leaderboard and member-sort filter lost their "Points" option - leaderboard is now Stars/Kills/KDR (was Stars/Points/Kills/KDR) and the member list filter is now Join Date/Kills/Name (was Join Date/Points/Name). `/mateam team <tag>` and `/team info` no longer show a Weekly Points line.

## 1.2.36 changes
- Added Discord slash commands `/link` and `/verify` (identical aliases) as a shortcut for the verification channel's **Verify Code** button, for anyone who'd rather not go find that channel and click it. Run with no arguments to get the same modal the button opens; run with `code:<your code>` to skip the modal entirely and verify straight off the command. Both share the exact same verification logic (expiry, eligibility, already-linked checks, role assignment) the button's modal already used. Not staff-gated - any guild member can use either. The verification panel embed and `/profile`'s "not linked" reply now mention this option too.

## 1.2.37 changes
Fixed a real bug behind `/profile` intermittently losing online status/Time Registered/Last Login even with a correctly-configured, actually-connected MakongVelocity: the website bridge only ever sent a server's `kind` (`paper` vs `velocity`) once, in its one-time `connect()` call - every poll after that (including forever, for the lifetime of the plugin process) only sent a heartbeat with no `kind`. The website's in-memory server registry resets on every restart/deploy, and defaults an unrecognized server id to `kind: "paper"` the instant *anything* references it (a poll, or another server's `/profile` request naming it as the target) - so any website restart permanently flipped an already-running Velocity proxy to showing as `paper` until that proxy itself was restarted, silently breaking `/profile`'s network lookup (it specifically looks for a connected `velocity`-kind server) with zero visible error on the plugin side. `WebsiteBridge#poll()` (`web/WebsiteBridge.java`, identical fix needed in MakongVelocity's own copy) now sends `kind` on every single poll, not just once, so the website's registry self-heals within one poll interval (~3s) of any restart instead of needing a manual plugin restart.

## 1.2.38 changes
- `/matier stats|set|add|remove|reset <player>` now work for offline players, not just whoever's currently online. `MaTierCommand` was looking players up with `Bukkit.getPlayerExact()` (online-only) even though `MaTierService`'s underlying Star storage was already fully UUID-based and never cared whether the target was online - switched to `Bukkit.getOfflinePlayer()`, which resolves anyone who has ever played on this server (and still rejects a name nobody's ever seen, same as before). `/matier resetall` and `/matier top` were unaffected - they never looked a specific player up by name.
...
## 1.2.41 changes
Added alt accounts: one Discord account may now link up to three Minecraft accounts - a **Main** plus up to two **Alts** - instead of exactly one.
- How many total accounts a Discord id may link is gated by its own age and time in your Discord server (`discord.alts.tiers` in `module/verification.yml`, shipped defaults: 1+ year old & 3+ months in the guild -> Main + Alt#1; 2+ years old & 9+ months -> Main + Alt#1 + Alt#2). This is **additive to** the existing `discord.guild.eligibility_tiers`, unchanged - that still separately gates whether a Discord account may link *anything* at all (its first/Main account); the new `alts.tiers` only controls how many more it can add on top. If you want the "first account" requirement itself changed (e.g. to just a 1-month-old account with no guild-membership minimum), edit `eligibility_tiers` directly - I left its shipped default as-is rather than guessing at a change to existing verification behavior.
- `/verify`/`/link`/the **Verify Code** button all work exactly the same for a 2nd or 3rd account as for the first - they now check the account-count cap instead of hard-rejecting any Discord id that already has something linked, and tell you which slot a successful link landed in.
...
- `/team ally <tag>` now creates a real, persisted alliance request. The target team's online owner/admins are notified in chat and can click the pending request in the team GUI's Allies screen to open an Accept/Deny confirmation (two separate buttons, no right-click needed - works the same on Bedrock/Floodgate as on Java), alongside the existing allied-teams list (click an entry to remove it).
...
## 1.2.45 changes
Added a live Discord network status feature (`module/verification.yml`'s new `discord.status` section - hub server only, off by default).
- `/status` - anyone can run it, posts an embed showing every configured server's live online/offline/starting/maintenance state and player count.
- An optional self-editing status panel: set `discord.status.enabled: true` and `discord.status.channel_id`, and this plugin posts one embed there and keeps re-editing it in place every `update_interval_seconds` (default 60) - same self-editing-message approach the verification panel already uses.
