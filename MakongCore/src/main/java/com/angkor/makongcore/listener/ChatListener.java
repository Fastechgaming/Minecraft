...
public final class ChatListener implements Listener {
    ...
    public static void beginCreate(Player p){
        INPUT.put(p.getUniqueId(),Step.TAG);
    ...
    private static void showNamePrompt(Player p){
        p.sendTitle("team name","type your team name in chat.",10,60,10);
        p.sendMessage(Text.mm("<yellow>set tag: <white>"+DATA.getOrDefault(p.getUniqueId(),"")+"</white>"));
    ...
    public void chat(AsyncPlayerChatEvent e){
        ...
        if(step!=null){
            ...
            if(step==Step.TAG){
                ...
                DATA.put(u,m);INPUT.put(u,Step.NAME);showNamePrompt(p);return;
            }
            if(step==Step.NAME){
                ...
                if(ts.byTag(DATA.get(u))!=null){
                    p.sendMessage(Text.mm("<red>Tag rejected: <white>that tag is already in use.</white></red>"));return;
                }
                String tag=DATA.remove(u);INPUT.remove(u);
                p.sendMessage(Text.mm("<yellow>set tag: <white>"+tag+"</white>"));
                p.sendMessage(Text.mm("<yellow>set name: <white>"+m+"</white>"));
                p.sendMessage(Text.mm("<green>creating team...</green>"));
                createAsync(p,tag,m);return;
            }
