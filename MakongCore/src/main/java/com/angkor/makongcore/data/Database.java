...
public final class Database implements Closeable {
 ...
 public Database(FileConfiguration c, java.io.File folder) throws Exception {String type=c.getString("storage.type","h2"); HikariConfig h=new HikariConfig(); h.setMaximumPoolSize(c.getInt("storage.mysql.max_pool_size",8));h.setMinimumIdle(c.getInt("storage.mysql.min_idle",1));h.setConnectionTimeout(c.getLong("storage.mysql.connection_timeout",10000));
  if(type.equalsIgnoreCase("mysql")){h.setDriverClassName("com.mysql.cj.jdbc.Driver");String host=c.getString("storage.mysql.host","localhost"),db=c.getString("storage.mysql.database","mateam");h.setJdbcUrl("jdbc:mysql://"+host+":"+c.getInt("storage.mysql.port",3306)+"/"+db+"?useSSL="+c.getBoolean("storage.mysql.use_ssl",false)+"&useUnicode=true&characterEncoding=UTF-8&connectionCollation=utf8mb4_unicode_ci&serverTimezone=UTC");h.setUsername(c.getString("storage.mysql.username","root"));h.setPassword(c.getString("storage.mysql.password",""));}
 ...
 // DIFFERENT server than the one the player is actually frozen on (Discord
 // dispatches interactions to every connected session for that token, and
 // only one can win the race to acknowledge). That server's local map won't
 // have the code, so it falls back to this table before giving up.
 public CompletableFuture<Void> putPending(String code,UUID uuid,String name,long expiresAt,String accountType,boolean discordOnly){return CompletableFuture.runAsync(()->{try(Connection c=ds.getConnection()){c.setAutoCommit(false);try{try(PreparedStatement d=c.prepareStatement("DELETE FROM pending_links WHERE uuid=?")){d.setString(1,uuid.toString());d.executeUpdate();}try(PreparedStatement p=c.prepareStatement("INSERT INTO pending_links(code,uuid,name,expires_at,account_type,discord_only) VALUES(?,?,?,?,?,?)")){p.setString(1,code);p.setString(2,uuid.toString());p.setString(3,name);p.setLong(4,expiresAt);p.setString(5,accountType);p.setBoolean(6,discordOnly);p.executeUpdate();}c.commit();}catch(Exception e){try{c.rollback();}catch(SQLException ignored){}throw e;}}catch(Exception e){throw new CompletionException(e);}},io);}
 public CompletableFuture<PendingLink> findPending(String code){return CompletableFuture.supplyAsync(()->{try(Connection c=ds.getConnection();PreparedStatement p=c.prepareStatement("SELECT * FROM pending_links WHERE code=?")){p.setString(1,code);try(ResultSet r=p.executeQuery()){if(!r.next())return null;long expiresAt=r.getLong("expires_at");if(expiresAt<System.currentTimeMillis())return null;return new PendingLink(UUID.fromString(r.getString("uuid")),r.getString("name"),expiresAt,r.getString("account_type"),r.getBoolean("discord_only"));}}catch(SQLException e){throw new CompletionException(e);}},io);}
 public CompletableFuture<Void> removePending(String code){return CompletableFuture.runAsync(()->{try(Connection c=ds.getConnection();PreparedStatement p=c.prepareStatement("DELETE FROM pending_links WHERE code=?")){p.setString(1,code);p.executeUpdate();}catch(SQLException e){throw new CompletionException(e);}},io);}
 public record PendingLink(UUID uuid,String name,long expiresAt,String accountType,boolean discordOnly){}

 // /malink bypass|unbypass|bypasslist - a persisted allowlist of players who
 // may skip the "must link before you can play" freeze entirely (see
 // AccountLinkService#onJoin's isBypassed check). Independent of
 // account_links - a bypassed player is never required to link, whether or
 // not they ever actually do.
 public CompletableFuture<Void> addBypass(UUID uuid,String name){return CompletableFuture.runAsync(()->{try(Connection c=ds.getConnection()){try(PreparedStatement d=c.prepareStatement("DELETE FROM link_bypass WHERE uuid=?")){d.setString(1,uuid.toString());d.executeUpdate();}try(PreparedStatement p=c.prepareStatement("INSERT INTO link_bypass(uuid,name,added_at) VALUES(?,?,?)")){p.setString(1,uuid.toString());p.setString(2,name);p.setLong(3,System.currentTimeMillis());p.executeUpdate();}}catch(SQLException e){throw new CompletionException(e);}},io);}
 public CompletableFuture<Boolean> removeBypass(UUID uuid){return CompletableFuture.supplyAsync(()->{try(Connection c=ds.getConnection();PreparedStatement p=c.prepareStatement("DELETE FROM link_bypass WHERE uuid=?")){p.setString(1,uuid.toString());return p.executeUpdate()>0;}catch(SQLException e){throw new CompletionException(e);}},io);}
 public CompletableFuture<Boolean> isBypassed(UUID uuid){return CompletableFuture.supplyAsync(()->{try(Connection c=ds.getConnection();PreparedStatement p=c.prepareStatement("SELECT 1 FROM link_bypass WHERE uuid=?")){p.setString(1,uuid.toString());try(ResultSet r=p.executeQuery()){return r.next();}}catch(SQLException e){throw new CompletionException(e);}},io);}
 public CompletableFuture<List<BypassEntry>> listBypass(){return CompletableFuture.supplyAsync(()->{List<BypassEntry> out=new ArrayList<>();try(Connection c=ds.getConnection();PreparedStatement p=c.prepareStatement("SELECT * FROM link_bypass ORDER BY added_at");ResultSet r=p.executeQuery()){while(r.next())out.add(new BypassEntry(UUID.fromString(r.getString("uuid")),r.getString("name"),r.getLong("added_at")));}catch(SQLException e){throw new CompletionException(e);}return out;},io);}
 public record BypassEntry(UUID uuid,String name,long addedAt){}

 // /makongcore reset mateam|matier|verification|all - full, irreversible
 // wipes of each module's persisted data (gated behind a confirm argument
 // at the command layer, see AdminCommand). Not the same as /matier
 // resetall (which only zeroes Stars) - this also clears matier_history,
 // i.e. every season's tier/rank record too.
 public CompletableFuture<Void> deleteAllTeams(){return CompletableFuture.runAsync(()->{try(Connection c=ds.getConnection();Statement s=c.createStatement()){s.executeUpdate("DELETE FROM team_allies");s.executeUpdate("DELETE FROM team_members");s.executeUpdate("DELETE FROM teams");}catch(SQLException e){throw new CompletionException(e);}},io);}
 public CompletableFuture<Void> wipeMatier(){return CompletableFuture.runAsync(()->{try(Connection c=ds.getConnection();Statement s=c.createStatement()){s.executeUpdate("DELETE FROM matier_players");s.executeUpdate("DELETE FROM matier_history");}catch(SQLException e){throw new CompletionException(e);}},io);}
