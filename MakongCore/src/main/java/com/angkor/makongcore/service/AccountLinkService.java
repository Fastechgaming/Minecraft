...
public final class AccountLinkService extends ListenerAdapter implements Listener, org.bukkit.plugin.messaging.PluginMessageListener {
    ...
    // Title/description/color/button-label all come from module/verification.yml
    // (discord.verification.embed.*) rather than being hardcoded, so an admin
    // can restyle the panel without touching Java. \n in the config text
    // becomes a real line break (Discord embeds don't render literal "\n").
    private net.dv8tion.jda.api.entities.MessageEmbed verificationEmbed(){
        String title=cfg.s("discord.verification.embed.title","Makong Minecraft Verification");
        String description=cfg.s("discord.verification.embed.description","Link your Minecraft account to Discord.\\n\\nClick **Verify Code** and enter the 6-digit code shown in Minecraft.").replace("\\n","\n");
        var builder=new net.dv8tion.jda.api.EmbedBuilder().setTitle(title).setDescription(description);
        String color=cfg.s("discord.verification.embed.color","").trim();
        if(!color.isBlank()){try{builder.setColor(java.awt.Color.decode(color.startsWith("#")?color:"#"+color));}catch(NumberFormatException ignored){}}
        return builder.build();
    }
    private Button verifyButton(){return Button.primary("makong:verify",cfg.s("discord.verification.embed.button_label","Verify Code"));}
    private void sendVerificationPanel(){String channelId=cfg.s("discord.verification.channel_id","");if(channelId.isBlank()||jda==null)return;TextChannel ch=jda.getTextChannelById(channelId);if(ch==null)return;var embed=verificationEmbed();String configured=cfg.s("discord.verification.panel_message_id","");if(!configured.isBlank()){editPanel(ch,configured,embed);return;}db.meta("discord_verification_panel_message_id").thenAccept(id->{if(id!=null&&!id.isBlank())editPanel(ch,id,embed);else ch.sendMessageEmbeds(embed).setComponents(ActionRow.of(verifyButton())).queue(msg->db.setMeta("discord_verification_panel_message_id",msg.getId()));});}
    private void editPanel(TextChannel ch,String id,net.dv8tion.jda.api.entities.MessageEmbed embed){ch.retrieveMessageById(id).queue(msg->msg.editMessageEmbeds(embed).setComponents(ActionRow.of(verifyButton())).queue(),err->ch.sendMessageEmbeds(embed).setComponents(ActionRow.of(verifyButton())).queue(msg->db.setMeta("discord_verification_panel_message_id",msg.getId())));}
