...
return t;}
 ...
 public boolean addMemberStats(Team t,UUID u,long points,long kills,long deaths,long minutes){
  if(t==null)return false;
  TeamMember m=t.member(u); if(m==null)return false;
  long nextPoints=Math.max(0,m.points()+points);
  ...
  t.addMember(new TeamMember(m.uuid(),m.name(),m.role(),m.joinedAt(),m.lastSeen(),m.server(),
      nextPoints,nextKills,nextDeaths,nextMinutes));
  long teamPoints=Math.max(0,t.points()+points);
  ...
  t.setStats(teamPoints,teamKills,teamDeaths,teamPlaytime);
  db.save(t);
  return true;
 }
 public boolean addMemberPoints(Team t,UUID u,long amount){
  if(t==null)return false;
  TeamMember m=t.member(u); if(m==null)return false;
  long next=Math.max(0,m.points()+amount);
  t.removeMember(u);
  t.addMember(new TeamMember(m.uuid(),m.name(),m.role(),m.joinedAt(),m.lastSeen(),m.server(),next));
  db.save(t); return true;
 }
 ...
 public CompletableFuture<Void> resetWeeklyPoints(){
        List<CompletableFuture<Void>> saves=new ArrayList<>();
        for(Team t:teams.values()){
            List<TeamMember> members=new ArrayList<>(t.members());
            for(TeamMember m:members){
                t.removeMember(m.uuid());
                t.addMember(new TeamMember(m.uuid(),m.name(),m.role(),m.joinedAt(),m.lastSeen(),m.server(),0,0,0,0));
            }
            t.setStats(0,0,0,0);
            saves.add(db.save(t));
        }
